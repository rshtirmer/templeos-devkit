...
def lint(path: str, src: str) -> list[Diag]:
    ...
    for idx, t in enumerate(code):
        ...
        # NOTE: boot-phase-return / boot-phase-goto / boot-phase-loop
        # used to live here as token-pattern heuristics. They moved to
        # the Rust parser (`holycc`), which catches the same conditions
        # with proper scope awareness — see parse_top_item /
        # parse_statement_top file-scope checks. The lint deliberately
        # no longer covers them: this tool now owns formatting + lex-
        # level + cheap heuristics; semantic rules are parser-owned.
        _ = (idx, t)
        break
    ...
    # NOTE: exponent-float-literal moved to the Rust parser's lexer
    # (Q1 in docs/lex-spec.md). `holycc` emits the same diagnostic
    # text with the same `[exponent-float-literal]` rule tag.

    # NOTE: comma-decl-list, multi-array-decl, f32-reference, and
    # reserved-name-collision moved to the Rust parser. Their lint
    # implementations were heuristic regex; the parser does the same
    # checks accurately against the AST. See feat/lint-parser-
    # specialization commit message for the specialization rationale.
