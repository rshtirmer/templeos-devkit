...
fn parse_statement_inner(p: &mut Parser, at_file_scope: bool) -> Option<Stmt> {
    ...
    match p.peek().clone() {
        ...
        TokenKind::StrLit(_) | TokenKind::CharLit(_) => {
            ...
            // expression list before the `;`. We model the whole
            // thing as an `ExprKind::Comma`-wrapped expression so the
            // statement carries every argument; downstream typeck
            // would lower it to a `Print` / `PutChars` call.
...
/// Parse an implicit-Print / PutChars statement: a leading string or
/// char literal optionally followed by `, expr, expr, ...`.
///
/// HolyC routes this through `Print` for `StrLit` and `PutChars` for
/// `CharLit` (parse-spec §5.13). We don't lower at parse time — we
/// produce `Stmt::Expr(...)` with the head expression alone if there
/// are no extra args, or `Stmt::Expr(Comma([head, args...]))` if any.
fn parse_implicit_print_stmt(p: &mut Parser, start: crate::lex::Pos) -> Option<Stmt> {
    use crate::parse::ast::{Expr, ExprKind};
    // Eat the leading literal directly so we never re-enter the
    // generic expression parser at the top — it would correctly parse
    // the literal but mis-handle the trailing `,` as an unexpected token.
    let head_tok = p.bump();
